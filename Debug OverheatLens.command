#!/bin/bash
# Double-click to start OverheatLens in DEBUG mode (macOS).
...
wd = Path(os.environ.get('OVERHEATLENS_WEATHER_DIR', '/Users/mohamedali/Library/CloudStorage/OneDrive-LeedsBeckettUniversity/Work/Ph.D/DataBase/DataBase/LEEDS Weather Files/Weather File MET Office'))
...
" 2>&1 | tee "$LOG"; then
  ...
  echo "Self-check hit a file-read stall (OneDrive still syncing?) — retrying ($attempt/3)…" | tee -a "$LOG"
  sleep 15
done
...
echo "  the OverheatLens folder in Finder → “Always Keep on This Device”."
echo ""
